...
clear;  help advect;  % Clear memory and print header
...
CFL = -c*dt/(2.*dx);  % Coefficient used by all schemes
coefflw = 2*CFL^2;    % Coefficient used by L-W scheme
...
for iStep=1:nStep  %% MAIN LOOP %%
  ...
  if( method == 1 )      %%% FTCS method %%%
    a(1:N) = a(1:N) + CFL*(a(ip)-a(im));  
  elseif( method == 2 )  %%% Lax method %%%
    a(1:N) = .5*(a(ip)+a(im)) + CFL*(a(ip)-a(im));   
